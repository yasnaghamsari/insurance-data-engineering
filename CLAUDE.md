...
### Downstream
Gold Delta tables → ClickHouse (`docker/clickhouse/init.sql` for schema, `clickhouse_loader.py` for data) → Metabase. Dashboard screenshots live in `images/`, referenced from `README.md`. Metabase configuration itself isn't code in this repo. `metabase_queries/` has one `.sql` file per dashboard card, grouped one folder per dashboard — the actual queries behind the screenshots, meant to be pasted straight into Metabase's SQL editor.
...
- `most_common_borough`/`most_common_zip_code` (in `build_{daily,weekly,monthly}_accidents`) used to be computed with `F.max()`, which picks the alphabetically/numerically largest value, not the actually most frequent one. `_modes_per_group()` does a real per-group frequency count (excluding nulls — rows with a missing borough outnumber any single named borough in about half the months, so an unfiltered mode would mostly report "missing" as the answer). Caught by inspecting the loaded ClickHouse data, not by a test — a good example of why "runs without errors" and "produces correct output" are different bars.
- `build_monthly_policies` joined issued/expired policies with a left join keyed off the issued side, which silently dropped any month with only expirations and no new issuances (e.g. the tail end of the sample data, once issuances stop but 1-year policies are still expiring). Now a full outer join — `gold_policies_monthly` went from 72 rows to 88 once this was fixed and rerun.
